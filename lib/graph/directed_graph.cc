...
int directed_graph::move_edge(edge_base *e, int x, int y)
{
	...
	// The following old implementation does not work with
	// the edge_pointer_cmp in vertex_base, changing the
	// source/target of an edge in place will destroy the
	// order of the in_edge and out_edge sets
	/*
	if(s != x)
	{
		vv[s]->remove_out_edge(e);
...
// Ke: the implementation appears to be incorrect, but it is not used anywhere
