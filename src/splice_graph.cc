...
void splice_graph::all_pair_vertices_reachability(std::vector<bool>& canReach)
{
    int nv = num_vertices();
    std::vector<int> q;
    std::vector<int> vd(nv);
    ...
    {
	int v = q[k++];
	canReach[nv * v + v] = true;
	std::vector<int> vReach;
	if(in_degree(v) > 0)
	{
	    for(int i = 0; i < k; ++i)
	    {
	...
	for(tie(it, itEd) = in_edges(v); it != itEd; ++it)
	{
	    int p = (*it)->source();
	    vd[p] -= 1;
	    if(vd[p] == 0) q.push_back(p);
	    for(int u : vReach)
	    {
		canReach[nv * p + u] = true;
	    }
	}
    }
}
