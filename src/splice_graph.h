...
public:
    ...
    // for finding independent edges (that are not connected by any path)
    // canReach is assumed to have been initialized to length
    // num_vertices * num_vertices
    // canReach[i][i] (actually canReach[num_vertices*i + i]) is always true
    // only works for DAG
    void all_pair_vertices_reachability(std::vector<bool>& canReach);
